function [A, G]=coareG36vn_He(u,zu,t,zt,rh,zq,P,ts,Rs,Rl,lat,zi,rain,Ss,cp,sigH,zrf_u,zrf_t,zrf_q,dHe)
%
...
%  dHe air-sea delta He, micro-atmospheres
%
...
Whf=max(1.6e-3*U10N.^1.1./(cp./U10N).^0.5,1e-4);      %  whitecap fraction Banner model
Whf2=max(5.0e-8*(usr.*sigH./visw).^0.9,1e-4) ;                  %%  Byron Re formula
...
G = HEcalc(ut,du,Cdn_10/1000,Cd,usr,ts,tkt,dHe,xlamx,rhoa,rhow,visw,alq,f,Acoef,Bcoef);
...
%------------------------------------------------------------------------------
function y=solHe(x)
%solubility and Schmidt number as function of temperature for He
...
%sc=schmidt number
r1=8.314;
a=[-34.626 43.0285 14.1391];
b=[ -0.04234 0.022624 -0.003312];
a0=[410.14 20.503 0.53175 0.0060111];
tsx=(x+273.16)/100;
s=a(1)+a(2)./tsx+a(3).*log(tsx)+34*(b(1)+b(2)*tsx+b(3).*tsx.*tsx);

al=exp(s)*1e3./tsx/100;%
sol=al*1e5./r1./tsx/100;
...
%------------------------------------------------------------------------------
function G=HEcalc(ut,du,Cdn_10,Cd,usr,ts,tkt,dHe,xlamx,rhoa,rhow,visw,alq,f,A,B)
...
%CO2 variables
s=solHe(ts);
solH=s(:,1);%mole/m^3/atm
alH=s(:,2);%dimensionless
scwH=s(:,3);%schmidt #, water
scaH=0.9;%schmidt #, air

%Woolf bubble parameterization
u10n=usr./sqrt(Cdn_10).*du./ut;
fd=alH.*(1+1./(14.*alH./scwH.^.5).^(1/1.2)).^1.2;%Woolf equation
...
rwo=ftan.*sqrt(rhow./rhoa).*(hw.*sqrt(scwH)+(log(.5./tkt)/.4));%water side
ra=ha.*sqrt(scaH)+1./sqrt(Cd)-5+.5*log(scaH)/.4; %air side

%Fairall et al. 1999 parameterization
usw=usr./sqrt(rhow./rhoa).*6./xlamx;
rw=1./(1./rwo+kbb./usr);
vtc=usr./(rw+ra.*alH);%bubble xfer velocity
vtco=usr./(rwo+ra.*alH);%non-bubble xfer velocity
vtc2=vtco+kbb;
sc_correction=sqrt(scwH./660);
kco2=vtc;
cfb=kco2.*dHe;                   %He flux in mmol/m2/s
k660=kco2.*sc_correction;

G=[rwo ra rw vtco vtc phi solH alH scwH vtc2 kbb cfb k660];
