...
%     AQDprofiles_up = AQDprofiles;
    index=find(abs(cellfun(@(x) x.Burst_Pressure(end)-x.Burst_Pressure(1),AQDprofiles_up))>0.7*z_max);
    ...
    for k = 1:length(index)
        ...
        m = corr1<80 | corr2<80 | corr3<80 | corr4<80;
        vele{1}(find(m == 1)) = nan;
        vele{2}(find(m == 1)) = nan;
        vele{3}(find(m == 1)) = nan;
        ...
        % new for SR extract vel at depth > -5m
        indd = find(dpth_temp_all>-5);
        velemcE_temp_all(indd) = nan;
        velemcN_temp_all(indd) = nan;
